...
We can better interpret the region output by summarising the proportion of each cell type in a region across the individuals. Looking at the composition of cell types in each region, comparing between prognostic outcome.  
...
In this workshop, we use the scFeatures package to generate a molecular representation for each individuals from the matrix of proteins x cells. The molecular representation is interpretable and hence facilitates downstream analysis of the individual. In general, scFeatures generates features across six categories representing different molecular views of cellular characteristics. These include:
...
The different types of features constructed enable a more comprehensive multi-view understanding of each individuals from a matrix of `proteins x cells`. By default, the function will generated a total of 13 different types of features (feature_types) shown below and stored them in a list. All generated feature types are stored in a matrix of `samples x features`. 
```
...
There are different ways you can use `scFeatures` to generate molecular representation for individuals and it requires the following information for spatial data. 
...
## What can we learn from scFeatures output?
...
### Association study - can we identify "differential expression" for feature of interest
...
We visualize the comparison using a volcano plot and a dotplot for the cell type specific expression feature. This is a type of scatter-plot that is used to quickly identify changes in large data sets and represent thesignificance (y-axes) versus effect size or fold-change (x-axes). 
...
Whilte we have provided the code below, to save time, **just load the prepared RDS file `classifyr_result_IMC.rds`** and we will focus on the interpretation in this workshop. 
...
## Do each individual requires a different collection of features ? Dario word it better ? 
...
Q8: Is spatial features important for predicting recurrance ? 
Q9: Does your conclusion from the question above holds for all individuals ?
</div>
